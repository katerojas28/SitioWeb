        ...
        class administradorControlador extends administradorModelo{
            ...
            public function agregar_administrador_controlador(){
                ...
                if($password1!=$password2){
                    $alerta = [
                        ...
                        "Texto"=>"Las contraseñas que acaba de ingresar no coinciden, por favor intenete nuevamente",
                        "Tipo"=>"error"
                    ];
                }else{
                    $consulta1 = mainModel::ejecutar_consulta_simple("SELECT AdminNum FROM admin WHERE AdminNum='$dni'");
                    if($consulta1->rowCount()>=1){
                        ...
                        if($ec>=1){
                            $alerta = [
                                "Alerta"=>"simple",
                                "Titulo"=>"Ocurrio un error inesperado",
                                "Texto"=>"El E-mail que acaba de ingresar ya se encuentra registrado en el sistema",
                                "Tipo"=>"error"
                            ];
                        }else{
                            ...
                            if($consulta3->rowCount()>=1){
                                ...
                                $codigo = mainModel::generar_codigo_aleatorio("IE", "9", $numero);
                                ...
                                if($guardarCuenta->rowCount()>=1){
                                    ...
                                    $gruardarAdmin=administradorModelo::agregar_administrador_modelo($dataAD);
                                    if($gruardarAdmin->rowCount()>=1){
                                        $alerta = [
                                            "Alerta"=>"limpiar",
                                            "Titulo"=>"Administrador Registrado",
                                            "Texto"=>"El administrador se registro con exito",
                                            "Tipo"=>"success"
                                        ];
                                    }else{
                                    ...
                                    $alerta = [
                                        "Alerta"=>"simple",
                                        "Titulo"=>"Ocurrio un error inesperado",
                                        "Texto"=>"No hemos podido registrar el administradorrrrrr",
                                        "Tipo"=>"error"
                                    ];
                                }
                            }
            ...
            public function paginador_administrador_controlador($pagina, $registros, $privilegio, $codigo, $busqueda){
                ...
                if(isset($busqueda) && $busqueda!=""){
                    ...
                    WHERE ((CuentaCodigo !='$codigo' AND  id!='1') AND (AdminNombre LIKE '%$busqueda%' OR AdminApellido LIKE '%$busqueda%' 
                    OR AdminNum LIKE '%$busqueda%'  OR AdminTelefono LIKE '%$busqueda%' ) ) ORDER BY AdminNombre ASC LIMIT $inicio,$registros";
            ...
            //Controlador para mostrar datos y hacer conteo
            ...
            public function datos_Aadministrador_controlador($tipo){
                $tipo=mainModel::limpiar_cadena($tipo);

                return administradorModelo::datos_Aadministrador_modelo($tipo);
            }
            ...
            public function actualizar_administrador_controlador(){
                ...
                $num=mainModel::limpiar_cadena($_POST['num-up']);
                ...
                if($DatosAdmin!=$DatosAdmin['AdminNum']){
                    $consulta1=mainModel::ejecutar_consulta_simple ("SELECT AdminNum FROM admin WHERE AdminNUM='$num' AND CuentaCodigo!='$cuenta'");
                    if($consulta1->rowCount()>=1){
                        $alerta = [
                            "Alerta"=>"simple",
                            "Titulo"=>"Ocurrió un error inesperado",
                            "Texto"=>"El número de la cédula que acaba de ingresar ya se encuentra registrado en el sistema",
                            "Tipo"=>"error"
                        ];
                        return mainModel::sweet_alert($alerta);
                        exit();
