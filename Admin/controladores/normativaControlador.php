<?php
        if($peticionAjax){
            require_once "../modelos/normativaModelo.php";
        }else{
            require_once "./modelos/normativaModelo.php";
        }

        class normativaControlador extends normativaModelo{  
            public function agregar_norma_controlador(){
                ...
                if (!isset($_POST["file-reg"])){
                    ...
                    if ( $tipArchivo==$permitidos && $_FILES['file-reg']['size'] <= $limite_kb * 1024){
                        ...
                        $inf=$queryV->rowCount();
                        if($inf==0){
                            $queryV1 = mainModel::ejecutar_consulta_simple("SELECT id FROM normatividad WHERE NormaNomArchivo='$nomArchivo'");
